...
# The cco2 magkotsios plot of the Fe/Ti ratio shows 4 distinct regions:
#  - Region 1: blue-reddish blob in bottom left at log(rho) ~ 6.1
#  - Region 2: bright fat yellow-orange blob at log(rho) ~ 6.4
#  - Region 3: extended blue-ish region spanning temp ~ 6 to 8
#  - Region 4: isolated linear blue region out at temp ~ 9.5
...
## The regions were *previously* defined using the Fe/Ti vs S_rad histogram:
...
## Turns out that in that scheme, regions 1 thru 3 are not well-separated

# Last modified 4/9/19 by Greg Vance
...
# See young/shared_files/magkotsios/cco2_magkotsios_feti.png
REG1_PTEMP_MIN = 3.3e9 # K
REG1_PTEMP_MAX = 4.2e9 # K
REG1_PDENS_MIN = 10.**5.9 # g cm^-3
REG1_PDENS_MAX = 10.**6.4 # g cm^-3
REG2_PTEMP_MIN = 4.2e9 # K
REG2_PTEMP_MAX = 5.3e9 # K
REG2_PDENS_MIN = 10.**6.1 # g cm^-3
REG2_PDENS_MAX = 10.**6.8 # g cm^-3
REG3_PTEMP_MIN = 5.3e9 # K
REG3_PTEMP_MAX = 8.4e9 # K
REG3_PDENS_MIN = 10.**6.6 # g cm^-3
REG3_PDENS_MAX = 10.**7.3 # g cm^-3
REG4_PTEMP_MIN = 8.4e9 # K
REG4_PTEMP_MAX = 10.1e9 # K
REG4_PDENS_MIN = 10.**6.5 # g cm^-3
REG4_PDENS_MAX = 10.**7.0 # g cm^-3
...
for i in xrange(pid.size):
	if REG1_PTEMP_MIN < peak_temp[i] < REG1_PTEMP_MAX \
		and REG1_PDENS_MIN < peak_rho[i] < REG1_PDENS_MAX:
		reg1_pids.append(pid[i])
	elif REG2_PTEMP_MIN < peak_temp[i] < REG2_PTEMP_MAX \
		and REG2_PDENS_MIN < peak_rho[i] < REG2_PDENS_MAX:
		reg2_pids.append(pid[i])
	elif REG3_PTEMP_MIN < peak_temp[i] < REG3_PTEMP_MAX \
		and REG3_PDENS_MIN < peak_rho[i] < REG3_PDENS_MAX:
		reg3_pids.append(pid[i])
	elif REG4_PTEMP_MIN < peak_temp[i] < REG4_PTEMP_MAX \
		and REG4_PDENS_MIN < peak_rho[i] < REG4_PDENS_MAX:
